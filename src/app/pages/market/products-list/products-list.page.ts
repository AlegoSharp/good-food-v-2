...
export class ProductsListPage implements OnInit {
  ...
  async getArticles(init = false){
    await this.formService.getList("/Article/ingr/a_pageSize=25&pageNumber=" + (this.currentSlideNumber - 1).toString()).toPromise().then(response => {
      this.ArticlesPrev = [];
    ...
    if(init){
      await this.formService.getList("/Article/ingr/a_pageSize=25&pageNumber=" + this.currentSlideNumber).toPromise().then(response => {
        this.Articles = [];
        (response as Array<Article>).forEach(element => {
